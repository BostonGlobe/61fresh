...
class CalaisResponse():
    ...
    def get_entities(self):
        arrayEntities = []
	for item in self.entities:
            arrayEntities.append("INSERT INTO test.tblentities(txtType, txtName, numRelevance) VALUES (\"%s:\",\"%s\",\"%.2f\"" % (item['_type'], item['name'], item['relevance']) + ");" )
        return arrayEntities
    
    def gets_entities(self):
        arrayEntities = []
	for item in self.entities:
            arrayEntities.append("%s d78h %s d78h %.2f" % (item['_type'], item['name'], item['relevance']))
        return arrayEntities
